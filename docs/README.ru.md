...
- 🌐 Опциональный перевод подсказок
...
| `CF_IS_TRANSLATE` | Включить перевод подсказок 🌐 | Строка | `"true"` или `"false"` | - |
...
3. Игнорируйте настройки System Message, предоставляемые приложением, так как Flux-API-Worker использует встроенное системное сообщение. 💬🚫
