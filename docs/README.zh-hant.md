...
| `EXTERNAL_MODEL` | 外部優化模型名稱 🤖 | 字符串 | `"gpt-3.5-turbo"` | - |
...
2. 輸入您設置的 API KEY。🔑
3. 無需理會應用提供的 System Message 設置，因為 Flux-API-Worker 使用內置的 System Message。💬🚫
...
3. 提示詞優化不工作時，請確認 `PROMPT_OPTIMIZATION` 設置為 'true' 且外部 API 配置無誤。🌐🔧
